...
export function createCaRuleHybridGenerator(
  config: RuleConfig,
  ...
  return {
    ...
    create({ grid, rng, options }) {
      ...
      const context: CaRuleContext = {
        grid,
        rng,
        caveMask: buildRuleMask(grid, rng, config.surviveMax),
        started: false,
        stack: [start],
      ...
      return {
        step: () => stepCaRule(context),
      };
    },
  };
}

function stepCaRule(context: CaRuleContext) {
  ...
  const pool = chooseFavored ? favored : candidates;
  ...
  for (let iteration = 0; iteration < 4; iteration += 1) {
    ...
    for (let y = 0; y < grid.height; y += 1) {
      for (let x = 0; x < grid.width; x += 1) {
        ...
        if (current[index] === 1) {
          next[index] = aliveNeighbors >= 1 && aliveNeighbors <= surviveMax ? 1 : 0;
        } else {
