...
interface WeightedAStarContext {
  grid: Grid;
  startIndex: number;
  goalIndex: number;
  started: boolean;
...
> = {
  ...
  create({ grid, options }) {
    ...
    const context: WeightedAStarContext = {
      grid,
    ...
    return {
      step: () => stepWeightedAStar(context),
    };
  },
};

function stepWeightedAStar(context: WeightedAStarContext) {
  ...
  if (!context.started) {
    ...
    context.fScore[context.startIndex] =
      HEURISTIC_WEIGHT *
      manhattan(context.grid.width, context.startIndex, context.goalIndex);
