...
interface AStarEuclideanContext {
  grid: Grid;
  startIndex: number;
  goalIndex: number;
  started: boolean;
...
export const aStarEuclideanSolver: SolverPlugin<
  SolverRunOptions,
  AlgorithmStepMeta
> = {
  id: "astar-euclidean",
  ...
  create({ grid, options }) {
    ...
    const context: AStarEuclideanContext = {
      grid,
    ...
    return {
      step: () => stepAStarEuclidean(context),
    };
  },
};

function stepAStarEuclidean(context: AStarEuclideanContext) {
  ...
  if (!context.started) {
    ...
    context.fScore[context.startIndex] = euclidean(
      context.grid.width,
      context.startIndex,
      context.goalIndex,
    );

    patches.push({ index: context.startIndex, overlaySet: OverlayFlag.Frontier });
  ...
  if (context.currentIndex !== -1) {
    patches.push({ index: context.currentIndex, overlayClear: OverlayFlag.Current });
  ...
  if (context.openFlags[current] === 1) {
    ...
    context.frontierSize -= 1;
    patches.push({ index: current, overlayClear: OverlayFlag.Frontier });
  }
  ...
  if (current === context.goalIndex) {
    ...
    patches.push({ index: current, overlayClear: OverlayFlag.Current });
    context.currentIndex = -1;
  ...
  for (const neighbor of getOpenNeighbors(context.grid, current)) {
    ...
    context.fScore[neighbor] =
      tentativeG + euclidean(context.grid.width, neighbor, context.goalIndex);
...
function euclidean(width: number, fromIndex: number, toIndex: number): number {
